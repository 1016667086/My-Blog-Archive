...
* [一、JAVA多线程并发](#JAVA_6)
* + - [4.4.1.JAVA 线程实现/创建方式](#441JAVA___8)
    - * [4.1.2.1. 继承 Thread 类](#4121____Thread__9)
      * [4.1.2.2. 实现 Runnable 接口 。](#4122___Runnable___23)
    - [4.1.2.3. ExecutorService 、Callable 、Future 有返回值 有返回值线程](#4123__ExecutorService_CallableClass_Future___47)
    - * [4.1.2.4. 基于线程池的方式](#4124___74)
      * [4.1.3. 4 种线程池](#413_4__99)
      * [4.1.3.1. newCachedThreadPool](#4131__newCachedThreadPool_104)
      * [4.1.3.2. newFixedThreadPool](#4132__newFixedThreadPool_112)
      * [4.1.3.3. newScheduledThreadPool](#4133__newScheduledThreadPool_124)
      * [4.1.3.4. newSingleThreadExecutor](#4134__newSingleThreadExecutor_145)
    - [4.1.4. 线程生命周期（状态）](#414__149)
    - * [4.1.4.1. 新建状态（NEW ）](#4141__NEW__159)
      * [4.1.4.2. 就绪状态（RUNNABLE ）：](#4142__RUNNABLE__165)
      * [4.1.4.3. 运行状态（RUNNING ）：](#4143__RUNNING__169)
      * [4.1.4.4. 阻塞状态（BLOCKED ）：](#4144__BLOCKED__173)
      * [4.1.4.5. 线程死亡（DEAD ）](#4145__DEAD__190)
    - [4.1.5. 终止线程 的4 种方式](#415__4__199)
    - * [4.1.5.1. 正常运行结束](#4151___200)
      * [4.1.5.2. 使用退出标志退出线程](#4152___202)
      * [4.1.5.3. Interrupt 方法结束线程](#4153__Interrupt___222)
      * [4.1.5.4. stop 方法终止线程 （线程不安全）](#4154__stop___251)
    - [4.1.6. sleep 与 与 wait 区别](#416_sleep___wait___260)
    - [4.1.7. start 与 与 run 区别](#417_start___run___270)
    - [4.1.8. JAVA 后台线程](#418_JAVA___279)
    - [4.1.9. JAVA 锁](#419_JAVA__293)
    - * [4.1.9.1. 乐观锁](#4191___294)
      * [4.1.9.2. 悲观锁](#4192___302)
      * [4.1.9.3. 自旋锁](#4193___308)
    - [自旋锁的优缺点](#_315)
    - * [4.1.9.4. Synchronized 同步锁](#4194__Synchronized___337)
      * + [Synchronized 实现](#Synchronized__357)
      * [4.1.9.5. ReentrantLock](#4195__ReentrantLock_377)
      * + [公平锁](#_409)
      * [4.1.9.6. Semaphore 信号量](#4196__Semaphore___457)
      * + [Semaphore 与 ReentrantLock](#Semaphore___ReentrantLock_479)
      * [4.1.9.7. AtomicInteger](#4197__AtomicInteger_487)
      * [4.1.9.8. 可重入锁 递归锁）](#4198____498)
      * [4.1.9.9. 公平锁与非公平锁](#4199___504)
      * [4.1.9.10. ReadWriteLock 读写锁](#41910_ReadWriteLock___513)
      * [4.1.9.11. 共享锁和独占锁](#41911___527)
      * [4.1.9.12. 重量级锁 （ Mutex Lock ）](#41912___Mutex_Lock__543)
      * [4.1.9.13. 轻量级锁](#41913___552)
      * [4.1.9.14. 偏向锁](#41914___562)
      * [4.1.9.15. 分段锁](#41915__571)
      * [4.1.9.16. 锁 锁优化 优化](#41916____573)
    - [4.1.10. 线程基本方法 基本方法](#4110____594)
    - * [4.1.10.1. 线程等待（wait ）](#41101__wait__598)
      * [4.1.10.2. 线程睡眠（sleep ）](#41102__sleep__601)
      * [4.1.10.3. 线程让步（yield ）](#41103__yield__604)
      * [4.1.10.4. 线程中断（interrupt ）](#41104__interrupt__608)
      * [4.1.10.5. Join 等待其他线程终止](#41105_Join___617)
      * [4.1.10.6. 为什么要用 join()方法 方法？ ？](#41106__join___619)
      * [4.1.10.7. 线程唤醒（notify ）](#41107__notify__631)
      * [4.1.10.8. 其他方法：](#41108___641)
    - [4.1.11. 线程上下文切换 上下文切换](#4111____656)
    - * [4.1.11.1. 进程](#41111__662)
      * [4.1.11.2. 上下文](#41112__664)
      * [4.1.11.3. 寄存器](#41113__667)
      * [4.1.11.4. 程序计数器](#41114__669)
      * [4.1.11.5. PCB-“切换桢”](#41115_PCB_672)
      * [4.1.11.6. 上下文切换的活动：](#41116___677)
      * [4.1.12.1. 同步锁](#41121___689)
      * [4.1.12.2. 死锁](#41122___693)
      * [4.1.13. 线程池原理 原理](#4113____695)
      * [4.1.13.1. 线程复用](#41131__699)
      * [4.1.13.2. 线程池的组成](#41132__705)
      * [4.1.13.3. 拒绝策略](#41133__734)
      * [4.1.13.4. Java 线程池工作过程](#41134_Java__747)
      * [4.1.14. JAVA 阻塞队列原理](#4114__JAVA___763)
      * [4.1.14.1. 阻塞队列的主要方法](#41141__770)
      * [4.1.14.2. Java 中的阻塞队列](#41142_Java__810)
      * [4.1.14.3. ArrayBlockingQueue（公平、非公平）](#41143_ArrayBlockingQueue_819)
      * [4.1.14.4. LinkedBlockingQueue（两个独立锁提高并发）](#41144_LinkedBlockingQueue_826)
      * [4.1.14.5. PriorityBlockingQueue（compareTo 排序实现优先）](#41145_PriorityBlockingQueuecompareTo__832)
      * [4.1.14.6. DelayQueue（缓存失效、定时任务 ）](#41146_DelayQueue__836)
      * [4.1.14.7. SynchronousQueue（不存储数据、可用于传递数据）](#41147_SynchronousQueue_845)
    - [4.1.14.8. LinkedTransferQueue](#41148_LinkedTransferQueue_851)
    - * [4.1.15. CyclicBarrier 、CountDownLatch 、Semaphore 的 的用法 用法](#4115__CyclicBarrier_CountDownLatch_Semaphore____875)
      * [4.1.15.1. CountDownLatch（线程计数器 ）](#41151_CountDownLatch__876)
      * [4.1.15.2. CyclicBarrier（回环栅栏-等待至 barrier 状态再全部同时执行）](#41152_CyclicBarrier_barrier__904)
      * [4.1.15.3. Semaphore（信号量-控制同时访问的线程个数）](#41153_Semaphore_944)
    - [4.1.16. volatile 关键字的作用 关键字的作用（变量可见性、禁止重排序） （变量可见性、禁止重排序）](#4116__volatile____1000)
    - [4.1.17. 如何在两个线程之间共享数据 如何在两个线程之间共享数据](#4117____1031)
    - [4.1.18. ThreadLocal 作用（ 作用（ 线程本地存储 ） ）](#4118__ThreadLocal______1122)
    - * [4.1.20.2. ConcurrentHashMap 分段锁](#41202_ConcurrentHashMap__1186)
    - [4.1.21. Java 中用到的线程调度](#4121__Java___1201)
    - * [4.1.21.1. 抢占式调度：](#41211__1202)
      * [4.1.21.2. 协同式调度：](#41212__1206)
      * [4.1.21.3. JVM 的线程调度实现（抢占式调度）](#41213_JVM__1212)
      * [4.1.21.4. 线程让出 cpu 的情况：](#41214__cpu__1216)
      * [4.1.22. 进程调度算法 进程调度算法](#4122____1221)
      * [4.1.22.1. 优先调度算法](#41221__1222)
      * [4.1.22.2. 高优先权优先调度算法](#41222__1234)
      * [4.1.22.3. 基于时间片的轮转调度算法](#41223__1261)
    - [4.1.23. 什么是 什么是 CAS（ （ 比较并交换-乐观锁机制-锁自旋 ） ）](#4123____CAS_____1286)
    - * [4.1.23.1. 概念及特性](#41231__1287)
      * [4.1.23.2. 原子包 java.util.concurrent.atomic（锁自旋）](#41232__javautilconcurrentatomic_1296)
      * [4.1.23.3. ABA 问题](#41233_ABA__1326)
* [写在最后](#_1336)
...
#### 4.1.2.3. ExecutorService 、Callable 、Future 有返回值 有返回值线程

有返回值的任务必须实现 Callable 接口，类似的，无返回值的任务必须 Runnable 接口。

执行Callable 任务后，可以获取一个 Future 的对象，在该对象上调用 get 就可以获取到 Callable 任务。
...
Java里面线程池的顶级接口是Executor ，但是严格意义上来说Executor并不是一个线程池，而是一个执行线程的工具。正真的线程池的接口是ExecutorService。

##### 4.1.3.1. newCachedThreadPool

创建一个可根据需要创建新线程的线程池，但是在以前的线程可用的时将重用他们。对于执行很多短期异步任务的程序而言，这些线程池通常可以提高程序性能。

调用execute将重用以前构造的线程（如果线程可用）。如果现有线程没有可用的，则创建一个新线程并添加到池子中。终止并从缓存中移除那些已经有60秒未被使用的线程。

因此，长时间保持空闲的线程池不会使用任何的资源。
...
Executors.newSingleThreadExecutor()返回的一个线程池  
 （这个线程池只有一个线程）。这个线程池可以在线程死后（或者发生后异常的时候）重新启动一个线程来代替原来的线程继续执行下去。
...
当程序使用 使用new关键字创建了一个线程之后，

该线程处于新建状态，此时仅由JVM为其分配内存，并初始化其成员变量的值。
...
3. 其他阻塞 (sleep/join)  
    ...
    当 sleep()状态超时、join()等待线程终止或者超时、或者 I/O  
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/3a33be05f9d8970c53ee8eeb9ed30127.png)
...
使用一个变量来控制循环，例如：  
 最直接的方法就是设一个boolean 类型的标志，并通过设置这个标志为 true或 false 来控制 while循环是否退出，代码示例：
...
#### 4.1.6. sleep 与 与 wait 区别

3. 对于 sleep()方法，我们首先要知道该方法是属于 Thread 类中的。而 wait()方法，则是属于Object 类中的。
4. sleep()方法导致了程序暂停执行指定的时间，让出 cpu 该其他线程，但是他的监控状态依然保持者，当指定的时间到了又会自动恢复运行状态。
5. 在调用 sleep()方法的过程中，线程不会释放对象锁。
6. 而当调用 wait()方法的时候，线程会放弃对象锁，进入等待此对象的等待锁定池，只有针对此对象调用 notify()方法后本线程才进入对象锁定池准备获取对象锁进入运行状态。

#### 4.1.7. start 与 与 run 区别

7. start（）方法来启动线程，真正实现了多线程运行。
...
* 线程则是 JVM 级别的，以 Tomcat 为例，如果你在 Web 应用中启动一个线程，这个线程的生命周期并不会和 Web 应用程序保持同步。
* 也就是说，即使你停止了 Web 应用，这个线程依旧是活跃的。
* example: 垃圾回收线程就是一个经典的守护线程，当我们的程序中不再有任何运行的Thread,程序就不会再产生垃圾，垃圾回收器也就无事可做，所以当垃圾回收线程是 JVM 上仅剩的线程时，垃圾回收线程会自动离开。
* 它始终在低级别的状态中运行，用于实时监控和管理系统中的可回收资源。
* 生命周期：守护进程（Daemon）是运行在后台的一种特殊进程。
* 它独立于控制终端并且周期性地执行某种任务或等待处理某些发生的事件。
* 也就是说守护线程不依赖于终端，但是依赖于系统，与系统“同生共死”。
* 当 JVM 中所有的线程都是守护线程的时候。
* JVM 就可以退出了；如果还有一个或以上的非守护线程则 JVM 不会退出。
...
* 所以不会上锁，但是在更新的时候会判断一下在此期间别人有没有去更新这个数据，采取在写时先读出当前版本号，然后加锁操作（比较跟上一次的版本号，如果一样则更新），如果失败则要重复读-比较-写的操作。
...
* 占着 XX 不 XX，同时有大量线程在竞争一个锁，会导致获取锁的时间很长，线程自旋的消耗大于线程阻塞挂起操作的消耗，其它需要 cup 的线程又不能获取到 cpu，造成 cpu 的浪费。
* 所以这种情况下我们要关闭自旋锁；自旋锁时间阈值 （ 1.6 引入了适应性自旋锁 ）自旋锁的目的是为了占着 CPU 的资源不释放，等到获取到锁立即进行处理。
* 但是如何去选择自旋的执行时间呢？如果自旋执行时间太长，会有大量的线程处于自旋状态占用 CPU 资源，进而会影响整体系统的性能。
* 因此自旋的周期选的额外重要！
* JVM 对于自旋周期的选择，jdk1.5 这个限度是一定的写死的，在 1.6 引入了适应性自旋锁。
* 适应性自旋锁意味着自旋的时间不在是固定的了，而是由前一次在同一个锁上的自旋时间以及锁的拥有者的状态来决定。
* 基本认为一个线程上下文切换的时间是最佳的一个时间，同时 JVM 还针对当前 CPU 的负荷情况做了较多的优化。
* 如果平均负载小于 CPUs 则一直自旋，如果有超过(CPUs/2)  
   个线程正在自旋，则后来线程直接阻塞，如果正在自旋的线程发现 Owner 发生了变化则延迟自旋时间（自旋计数）或进入阻塞。
* 如果 CPU 处于节电模式则停止自旋，自旋时间的最坏情况是 CPU的存储延迟（CPU A 存储了一个数据，到 CPU B 得知这个数据直接的时间差），自旋时会适当放弃线程优先级之间的差异。
...
18. 作用于方法时，锁住的是对象的实例(this)；
19. 当作用于静态方法时，锁住的是Class实例，又因为Class的相关数据存储在永久带PermGen  
     （jdk1.8 则是 metaspace），永久带是全局共享的，因此静态方法锁相当于类的一个全局锁，  
     会锁所有调用该方法的线程；
20. synchronized 作用于一个对象实例时，锁住的是所有以该对象为锁的代码块。它有多个队列，当多个线程一起访问某个对象监视器的时候，对象监视器会将这些线程存储在不同的容器中。
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/c058ff1df47b94b38963086c0f9784ea.png)

1. JVM 每次从队列的尾部取出一个数据用于锁竞争候选者（OnDeck），但是并发情况下，ContentionList 会被大量的并发线程进行 CAS 访问，为了降低对尾部元素的竞争，JVM 会将  
...
> 参考：https://blog.csdn.net/zqz\_zqz/article/details/70233767
...
ReentantLock 继承接口 Lock 并实现了接口中定义的方法，他是一种可重入锁，除了能完成 synchronized 所能完成的所有工作外，还提供了诸如可响应中断锁、可轮询锁请求、定时锁等避免多线程死锁的方法。Lock 接口的主要方法：

1. void lock(): 执行此方法时, 如果锁处于空闲状态, 当前线程将获取到锁. 相反, 如果锁已经  
    被其他线程持有, 将禁用当前线程, 直到当前线程获取到锁.
2. boolean tryLock()：如果锁可用, 则获取锁, 并立即返回 true, 否则返回 false. 该方法和lock()的区别在于, tryLock()只是"试图"获取锁, 如果锁不可用, 不会导致当前线程被禁用,  
    当前线程仍然继续往下执行代码. 而 lock()方法则是一定要获取到锁, 如果锁不可用, 就一直等待, 在未获得锁之前,当前线程并不继续向下执行.
...
1. ReentrantLock 通过方法 lock()与 unlock()来进行加锁与解锁操作，与 synchronized 会被 JVM 自动解锁机制不同，ReentrantLock 加锁后需要手动进行解锁。为了避免程序出  
...
4. ReentrantLock 类可以唤醒指定条件的线程，而 object 的唤醒是随机的。tryLock 和 lock 和 lockInterruptibly 的区别
...
Semaphore 是一种基于计数的信号量。它可以设定一个阈值，基于此，多个线程竞争获取许可信号，做完自己的申请后归还，超过阈值后，线程申请许可信号将会被阻塞。Semaphore 可以用来  
 构建一些对象池，资源池之类的，比如数据库连接池实现互斥锁（计数器为 1 ）我们也可以创建计数为 1 的 Semaphore，将其作为一种类似互斥锁的机制，这也叫二元信号量，表示两种互斥状态。代码实现  
 它的用法如下：
...
令人兴奋地，还可以通过 AtomicReference将一个对象的所有操作转化成原子操作。

我们知道，在多线程程序中，诸如++i 或 i++等运算不具有原子性，是不安全的线程操作之一。

通常我们会使用 synchronized 将该操作变成一个原子操作，但 JVM 为此类操作特意提供了一些同步类，使得使用更方便，且使程序运行效率变得更高。

通过相关资料显示，通常AtomicInteger的性能是 ReentantLock 的好几倍。

##### 4.1.9.8. 可重入锁 递归锁）

本文里面讲的是广义上的可重入锁，而不是单指JAVA 下的 ReentrantLock。可重入锁，也叫做递归锁，指的是同一线程 外层函数获得锁之后 ，内层递归函数仍然有获取该锁的代码，但不受  
 影响。
...
JDK 中对 Synchronized 做的种种优化，其核心都是为了减少这种重量级锁的使用。JDK1.6 以后，为了减少获得锁和释放锁所带来的性能消耗，提高性能，引入了“轻量级锁”和  
 “偏向锁”。
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/cf078089778ecfe41674b974b0c083e2.png)
...
Object 类中的 notify() 方法，唤醒在此对象监视器上等待的单个线程，如果所有线程都在此对象上等待。

则会选择唤醒其中一个线程，选择是任意的，并在对实现做出决定时发生，线程通过调

用其中一个 wait() 方法，在对象的监视器上等待，直到当前的线程放弃此对象上的锁定，才能继续执行被唤醒的线程，被唤醒的线程将以常规方式与在该对象上主动同步的其他所有线程进行竞争。
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/316821b7b8cb7032fc9a8961d2c75757.png)
...
3. 跳转到程序计数器所指向的位置（即跳转到进程被中断时的代码行），以恢复该进程在程序  
    中。  
    4.1.11.7. 引起线程上下文切换 引起线程上下文切换的原因 的原因
...
4. 任务队列：用于存放待处理的任务，提供一种缓冲机制  
    ...
    ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/fb1a447070e1e5016c3930e356ed0ca9.png)  
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/50bc6bd85af0fcf41c31f95d08e40eba.png)
...
基于链表的阻塞队列，同ArrayListBlockingQueue类似，此队列按照先进先出（FIFO）的原则对  
 ...
 LinkedBlockingQueue 会默认一个类似无限大小的容量（Integer.MAX\_VALUE）。
...
比 sychronized 更轻量级的同步锁  
 在访问 volatile 变量时不会执行加锁操作，因此也就不会使执行线程阻塞，因此 volatile 变量是一种比 sychronized 关键字更轻量级的同步机制。volatile 适合这种场景：一个变量被多个线程共  
 享，线程直接给这个变量赋值。
...
9. 底层实现不一样， synchronized 是同步阻塞，使用的是悲观并发策略，lock 是同步非阻  
    塞，采用的是乐观并发策略
10. Lock 是一个接口，而 synchronized 是 Java 中的关键字，synchronized 是内置的语言  
     实现。
...
比如说一个线程 one 从内存位置 V 中取出 A，这时候另一个线程 two 也从内存中取出 A，并且two 进行了一些操作变成了 B，然后 two 又将 V 位置的数据变成 A，这时候线程 one 进行 CAS 操  
