...
![外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传](https://img-home.csdnimg.cn/images/20230724024159.png?origin_url=%2Fassets%2Fimage-20240907150639348.png&pos_id=img-OCXnPhFK-1774262906822)
...
![外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传](https://img-home.csdnimg.cn/images/20230724024159.png?origin_url=assets%2Fimage-20240618153540896.png&pos_id=img-PubGngH1-1774262906824)
...
* Windows安装
  ...
  ![外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传](https://img-home.csdnimg.cn/images/20230724024159.png?origin_url=assets%2F1739642839675.png&pos_id=img-mvkG33CY-1774262906825)
* 验证安装成功
  ...
  ![外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传](https://img-home.csdnimg.cn/images/20230724024159.png?origin_url=assets%2F1739641908819.png&pos_id=img-YI3an0xC-1774262906825)
...
![外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传](https://img-home.csdnimg.cn/images/20230724024159.png?origin_url=%2Fassets%2Fg5.png&pos_id=img-jbTmB6nV-1774262906825)
...
| --- | --- | --- | --- |
| num\_ctx | 设置上下文token大小. (默认: 2048) | int | num\_ctx 4096 |
| repeat\_last\_n | 设置模型要回顾的距离以防止重复. (默认: 64, 0 = 禁用, -1 = num\_ctx) | int | repeat\_last\_n 64 |
| repeat\_penalty | 设置惩罚重复的强度。较高的值（例如，1.5)将更强烈地惩罚重复，而较低值（例如，0.9)会更加宽容。（默认值：1.1） | float | repeat\_penalty 1.1 |
...
| num\_predict | 生成文本时要预测的最大标记数。（默认值：128，-1 =无限生成，-2 =填充上下文） | int | num\_predict 42 |
| top\_k | **减少产生无意义的可能性。较高的值（例如100）将给出更多样化的答案，而较低的值（例如10）将更加保守。（默认值：40）** | int | top\_k 40 |
| top\_p | 与Top-K合作。较高的值（例如，0.95）将导致更多样化的文本，而较低的值（例如，0.5)将产生更集中和保守的文本。（默认值：0.9） | float | top\_p 0.9 |
| num\_gpu | 设置缓存到GPU显存中的模型层数 | int | 自动计算 |
...
![外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传](https://img-home.csdnimg.cn/images/20230724024159.png?origin_url=%2Fassets%2Fg6.png&pos_id=img-QwiEaGsy-1774262906826)
...
![外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传](https://img-home.csdnimg.cn/images/20230724024159.png?origin_url=%2Fassets%2Fbg-7.png&pos_id=img-Na5CFJff-1774262906826)

**Step 2：选择导入的文件**

![外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传](https://img-home.csdnimg.cn/images/20230724024159.png?origin_url=%2Fassets%2Fbg-9.png&pos_id=img-ADOA1E9r-1774262906826)

**Step 3：输入项目名称**

![外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传](https://img-home.csdnimg.cn/images/20230724024159.png?origin_url=%2Fassets%2Fbg-10.png&pos_id=img-TQroAPVk-1774262906826)

**Step 4：完成导入，进入项目**

> 中间如果有导入预览，则直接点击确定即可。

![外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传](https://img-home.csdnimg.cn/images/20230724024159.png?origin_url=%2Fassets%2Fimage-20240620203700320.png&pos_id=img-fv60QcrZ-1774262906827)

##### 配置环境地址

Oallma支持的API可以在资料文件夹中通过《Ollama API文档.html》了解详解，双击打开查看：

![外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传](https://img-home.csdnimg.cn/images/20230724024159.png?origin_url=%2Fassets%2Fimage-20240620174314154.png&pos_id=img-3JRWo3fs-1774262906827)

通过网页可以了解到Ollama支持7个API （这里只列举了常用的），接下来我们重点先了解对话和向量化接口，因为这两个接口是最重要的，其它接口则留给大家课后自行尝试，但是在正式体验之前，需要先配置一下环境地址。

**配置测试环境地址：**

![外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传](https://img-home.csdnimg.cn/images/20230724024159.png?origin_url=%2Fassets%2Fbg41.png&pos_id=img-6o9Eo4bO-1774262906827)
...
| keep\_alive | body | string | 否 | 模型内存保持时间 | 5m |
| tools | body | [object] | 否 | 工具 |  |
| options | body | object | 否 | 配置参数 | none |
| seed | body | integer | 否 | 生成种子 | none |
| top\_k | body | integer | 否 | 多样度 | 越高越多样，默认40 |
| top\_p | body | number | 否 | 保守度 | 越低越保守，默认0.9 |
| repeat\_last\_n | body | integer | 否 | 防重复回顾距离 | 默认: 64, 0 = 禁用, -1 = num\_ctx |
| temperature | body | number | 否 | 温度值 | 越高创造性越强，默认0.8 |
| repeat\_penalty | body | number | 否 | 重复惩罚强度 | 越高惩罚越强，默认1.1 |
...
| created\_at | string | true | none | 响应时间 | none |
| message | object | true | none | 响应内容 | none |
| role | string | true | none | 角色 | none |
| content | string | true | none | 内容 | none |
| tool\_calls | [object] | false | none | 调用的工具集 |  |
| done | boolean | false | none |  | none |
| total\_duration | integer | false | none | 总耗时 | none |
| load\_duration | integer | false | none | 模型加载耗时 | none |
| prompt\_eval\_count | integer | false | none | 提示词token消耗数 | none |
| prompt\_eval\_duration | integer | false | none | 提示词耗时 | none |
| eval\_count | integer | false | none | 响应token消耗数 | none |
| eval\_duration | integer | false | none | 响应耗时 | none |

* 对话操作演示

![外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传](https://img-home.csdnimg.cn/images/20230724024159.png?origin_url=%2Fassets%2Fimage-20240620212328483.png&pos_id=img-8herycWl-1774262906827)
...
* 重排模型：用于向量化数据的优化增强，典型的使用场景是：模型微调
  ...
  bce-reranker-base\_v1、bge-reranker-large、bge-reranker-v2-gemma、bge-reranker-v2-m3
...
![外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传](https://img-home.csdnimg.cn/images/20230724024159.png?origin_url=assets%2Fpb.png&pos_id=img-ySBvw9pw-1774262906828)
...
* 识别的结果为引文，需要自行翻译
* 避坑: images:[“里面写上转码后的图片”]

![外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传](https://img-home.csdnimg.cn/images/20230724024159.png?origin_url=%2Fassets%2Fimage-20240622163209620.png&pos_id=img-jG4SrsNc-1774262906828)
...
![外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传](https://img-home.csdnimg.cn/images/20230724024159.png?origin_url=%2Fassets%2Fimage-20240620215522035.png&pos_id=img-0k26NWUj-1774262906828)
...
![外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传](https://img-home.csdnimg.cn/images/20230724024159.png?origin_url=%2Fassets%2F1739203773266.png&pos_id=img-pctU4q5J-1774262906828)
...
ChatBox提供了windows桌面安装方式，相关文件已下载到02\_资料/Chatbox-1.9.8-Setup.exe，可以找到文件，然后通过以下操作进行安装：

##### 

\*\*Step 1：桌面win安装 \*\*

![外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传](https://img-home.csdnimg.cn/images/20230724024159.png?origin_url=assets%2F1739643125906.png&pos_id=img-1VM1ikfC-1774262906828)

**Step 2：访问ChatBox**

![外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传](https://img-home.csdnimg.cn/images/20230724024159.png?origin_url=%2Fassets%2F1739389459468.png&pos_id=img-9g6ZAOaP-1774262906829)
...
![外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传](https://img-home.csdnimg.cn/images/20230724024159.png?origin_url=%2Fassets%2F1739207562004.png&pos_id=img-tWBefnbL-1774262906829)
...
![外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传](https://img-home.csdnimg.cn/images/20230724024159.png?origin_url=%2Fassets%2F1739388676991.png&pos_id=img-owEJPuFc-1774262906829)
...
![外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传](https://img-home.csdnimg.cn/images/20230724024159.png?origin_url=%2Fassets%2Fca9a21bc-ea6c-4c90-bf4a-fa53b4fb2b5c&pos_id=img-xTpzeBWk-1774262906829)
...
![外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传](https://img-home.csdnimg.cn/images/20230724024159.png?origin_url=%2Fassets%2F1739206990140.png&pos_id=img-j6JjypEd-1774262906829)

**Step 3：开始对话**

配置完成之后，返回对话界面，在1号位置选择通义大模型，然后即可开始对话聊天。

![外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传](https://img-home.csdnimg.cn/images/20230724024159.png?origin_url=assets%2F1739206253326.png&pos_id=img-gKpLBkMe-1774262906829)
...
![外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传](https://img-home.csdnimg.cn/images/20230724024159.png?origin_url=assets%2F1739643461782.png&pos_id=img-8hJJVCzY-1774262906829)

AI 创业公司，旨在提供更深层次的会话理解。
...
[外链图片转存中…(img-xTpzeBWk-1774262906829)]
...
[外链图片转存中…(img-j6JjypEd-1774262906829)]

**Step 3：开始对话**

配置完成之后，返回对话界面，在1号位置选择通义大模型，然后即可开始对话聊天。

[外链图片转存中…(img-gKpLBkMe-1774262906829)]
...
[外链图片转存中…(img-8hJJVCzY-1774262906829)]